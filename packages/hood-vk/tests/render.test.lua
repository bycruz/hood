	...
	return ctx.device:createPipeline({
		...
		fragment = {
			...
			targets = { {
				...
				writeMask = core.ColorWrites.All,
			} },
		},
...
test.it("render: writeMask Color suppresses the alpha channel", function()
	local maskPipeline = ctx.device:createPipeline({
		...
		fragment = {
			...
			targets = { {
				format = "rgba8unorm",
				writeMask = core.ColorWrites.Color,  -- RGB only, no alpha write
			} },
		},
	})
...
local instVertexLayout = core.VertexLayout.new()
	:withAttribute({ type = "f32", size = 3, offset = 0 })

-- Per-instance data: vec2 offset + vec4 colour, 24 bytes.
local instanceLayout = core.VertexLayout.new()
...
local indirectVertexLayout = core.VertexLayout.new()
	:withAttribute({ type = "f32", size = 3, offset = 0 })

local indirectInstanceLayout = core.VertexLayout.new()
	:withAttribute({ type = "f32", size = 2, offset = 0 })
	:withAttribute({ type = "f32", size = 4, offset = 8 })
	:withInstanceRate()
